  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const maybeDanger = JSX8__namespace.findPropInAttributes(node.openingElement.attributes, initialScope)(
          "dangerouslySetInnerHTML"
        );
        if (eff.O.isSome(maybeDanger)) {
          context.report({
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const initialScope = context.sourceCode.getScope(node);
        const maybeTypeAttribute = JSX8__namespace.findPropInAttributes(attributes, initialScope)("type");
        if (eff.O.isSome(maybeTypeAttribute)) return;
        context.report({
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const { attributes } = node.openingElement;
        const initialScope = context.sourceCode.getScope(node);
        const maybeSandboxAttribute = JSX8__namespace.findPropInAttributes(attributes, initialScope)("sandbox");
        if (eff.O.isNone(maybeSandboxAttribute)) {
          context.report({
        ...
        const hasValidSandbox = eff.F.pipe(
          JSX8__namespace.getPropValue(sandboxAttribute, context.sourceCode.getScope(sandboxAttribute)),
...
function testReactVersion(context, comparator, version2) {
  const { version: localVersion } = shared.normalizeSettings(shared.decodeSettings(context.settings));
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const initialScope = context.sourceCode.getScope(node);
        const maybeSandboxAttribute = JSX8__namespace.findPropInAttributes(attributes, initialScope)("sandbox");
        if (eff.O.isNone(maybeSandboxAttribute)) return;
        const isSafeSandboxValue = !eff.F.pipe(
          JSX8__namespace.getPropValue(maybeSandboxAttribute.value, context.sourceCode.getScope(maybeSandboxAttribute.value)),
