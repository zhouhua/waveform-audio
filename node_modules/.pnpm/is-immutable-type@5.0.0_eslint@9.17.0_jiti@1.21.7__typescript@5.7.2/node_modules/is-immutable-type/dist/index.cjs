...
function isTypeReferenceWithTypeArguments(type) {
    return (tsApiUtils.isTypeReference(type) &&
...
function getTypeImmutabilityHelper(program, td, overrides, useCache, maxImmutability, typeMatchesPatternSpecifier) {
    ...
    let m_PreviousImmutability = exports.Immutability.Unknown;
    let m_state;
    do {
        m_state = m_stack.pop();
        ...
        if (m_state.immutability !== exports.Immutability.Calculating) {
...
function taskTriage(parameters, m_stack, m_state) {
    ...
    cacheData(parameters.program, parameters.cache, m_state.typeData, m_state.immutability);
    if (tsApiUtils.isUnionType(m_state.typeData.type)) {
        handleTypeUnion(m_stack, m_state);
        return;
    }
    if (tsApiUtils.isIntersectionType(m_state.typeData.type)) {
        handleTypeIntersection(parameters, m_stack, m_state);
        return;
    }
    if (tsApiUtils.isConditionalType(m_state.typeData.type)) {
    ...
    if (tsApiUtils.isObjectType(m_state.typeData.type)) {
...
function taskObjectIndexSignature(parameters, m_stack, m_state) {
    const [types, typeNodes] = tsApiUtils.isIntersectionType(m_state.typeData.type)
        ? [
...
function handleTypeFunction(m_state) {
    m_state.immutability = exports.Immutability.Immutable;
}
/**
...
function handleTypeTuple(parameters, m_stack, m_state) {
    if (!m_state.typeData.type.target.readonly) {
        m_state.immutability = exports.Immutability.Mutable;
        return;
    }
    handleTypeArray(parameters, m_stack, m_state);
}
...
function handleTypeObject(parameters, m_stack, m_state) {
    ...
    if (properties.length > 0) {
        for (const property of properties) {
            if (tsApiUtils.isPropertyReadonlyInType(m_stateWithLimits.typeData.type, property.getEscapedName(), checker) ||
            ...
            if (declarations.length > 0) {
                if (declarations.some((declaration) => hasSymbol(declaration) &&
                    ...
                    m_stateWithLimits.limits.max = min(m_stateWithLimits.limits.max, exports.Immutability.ReadonlyDeep);
                    continue;
                }
            ...
            m_stateWithLimits.immutability = exports.Immutability.Mutable;
            return;
        }
    }
    const propertyNodes = new Map(m_stateWithLimits.typeData.typeNode !== null &&
        tsApiUtils.hasType(m_stateWithLimits.typeData.typeNode) &&
    ...
    const children = properties
        .map((property) => {
        const propertyType = typeUtils.getTypeOfPropertyOfType(checker, m_stateWithLimits.typeData.type, property);
    ...
    if (children.length > 0) {
        m_stateWithLimits.limits.min = exports.Immutability.ReadonlyShallow;
...
function createIndexSignatureTaskStates(parameters, m_state, kind, typeData) {
    ...
    if (indexInfo === undefined) {
        m_state.immutability = exports.Immutability.Unknown;
        return [];
    }
    if (parameters.immutabilityLimits.max <= exports.Immutability.ReadonlyShallow) {
        m_state.immutability = exports.Immutability.ReadonlyShallow;
        return [];
    }
    if (indexInfo.isReadonly) {
        ...
        return [
            createChildrenReducerTaskState(m_state, [{ immutability: exports.Immutability.ReadonlyShallow }, child], max),
            child,
        ];
    }
    m_state.immutability = exports.Immutability.Mutable;
