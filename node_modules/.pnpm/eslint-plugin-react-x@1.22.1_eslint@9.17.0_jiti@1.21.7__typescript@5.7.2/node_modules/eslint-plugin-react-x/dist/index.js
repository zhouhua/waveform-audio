  ...
  create(context) {
    function getReportDescriptor(node) {
      return node.value ? eff.O.none() : eff.O.some({
        ...
        node,
        data: {
          propName: JSX8__namespace.getPropName(node)
        },
...
function isUsingReactChildren(node, context) {
  ...
  if (callee.object.type === types.AST_NODE_TYPES.MemberExpression && "name" in callee.object.object) {
    return core.isInitializedFromReact(callee.object.object.name, initialScope, { ...settings, strictImportCheck: true });
  }
  return false;
}
...
var no_children_prop_default = createRule({
  ...
  create(context) {
    return {
      JSXElement(node) {
        const initialScope = context.sourceCode.getScope(node);
        const prop = JSX8__namespace.getProp(node.openingElement.attributes, "children", initialScope);
...
var no_context_provider_default = createRule({
  ...
  create(context) {
    ...
    const { version: version2 } = shared.normalizeSettings(shared.decodeSettings(context.settings));
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (eff.O.isSome(AST11__namespace.traverseUp(node, core.isClassComponent))) return;
...
var no_direct_mutation_state_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      ...
      if (eff.O.isNone(maybeParentClass)) return eff.O.none();
      const parentClass = maybeParentClass.value;
      if (!core.isClassComponent(parentClass)) return eff.O.none();
      const maybeParentConstructor = AST11__namespace.traverseUpGuard(node, isConstructorFunction);
      if (eff.O.exists(maybeParentConstructor, (n) => context.sourceCode.getScope(node).block === n)) return eff.O.none();
      return eff.O.some({
...
var no_duplicate_key_default = createRule({
  ...
  name: RULE_NAME24,
  create(context) {
    const isWithinChildrenToArrayRef = eff.MutRef.make(false);
    ...
    function checkIteratorElement(node) {
      if (node.type !== types.AST_NODE_TYPES.JSXElement) return eff.O.none();
      const initialScope = context.sourceCode.getScope(node);
      return eff.F.pipe(
        JSX8__namespace.findPropInAttributes(node.openingElement.attributes, initialScope)("key"),
        eff.O.flatMap((k) => "value" in k ? eff.O.fromNullable(k.value) : eff.O.none()),
        eff.O.flatMap((v) => {
          return isKeyEqual(v, v) ? eff.O.some({
      ...
      switch (node.type) {
        case types.AST_NODE_TYPES.ConditionalExpression:
          ...
          return eff.F.pipe(
            ...
            eff.O.orElse(() => checkIteratorElement(node.alternate))
          );
        ...
        case types.AST_NODE_TYPES.LogicalExpression:
          ...
          return eff.F.pipe(
            ...
            eff.O.orElse(() => checkIteratorElement(node.right))
          );
        default:
          return eff.O.none();
      }
    ...
    function checkBlockStatement(node) {
      return AST11__namespace.getNestedReturnStatements(node).reduce((acc, statement) => {
        ...
        if (eff.O.isNone(maybeDescriptor)) return acc;
    ...
    return {
      "ArrayExpression, JSXElement > JSXElement"(node) {
        if (eff.MutRef.get(isWithinChildrenToArrayRef)) return;
      ...
      CallExpression(node) {
        if (core.isChildrenToArrayCall(node, context)) eff.MutRef.set(isWithinChildrenToArrayRef, true);
      ...
      "CallExpression:exit"(node) {
        if (core.isChildrenToArrayCall(node, context)) eff.MutRef.set(isWithinChildrenToArrayRef, false);
      }
    };
  },
  defaultOptions: []
...
var no_implicit_key_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      const initialScope = context.sourceCode.getScope(node);
      const keyPropFound = JSX8__namespace.findPropInAttributes(node.attributes, initialScope)("key");
  ...
  create(context) {
    ...
    function checkIteratorElement(node) {
      switch (node.type) {
        case types.AST_NODE_TYPES.JSXElement: {
          const initialScope = context.sourceCode.getScope(node);
          if (!JSX8__namespace.hasProp(node.openingElement.attributes, "key", initialScope)) {
            return eff.O.some({
    ...
    return {
      ArrayExpression(node) {
        ...
        for (const element of elements) {
          if (!JSX8__namespace.hasProp(element.openingElement.attributes, "key", initialScope)) {
            context.report({
              messageId: "noMissingKey",
              node: element
            });
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        for (const { name: componentName, node: component } of functionComponents) {
          ...
          const isInsideJSXPropValue = component.parent.type === types.AST_NODE_TYPES.JSXAttribute || eff.O.isSome(JSX8__namespace.traverseUpProp(node, (n) => n.value?.type === types.AST_NODE_TYPES.JSXExpressionContainer));
          ...
          if (eff.O.isSome(maybeParentComponent) && !core.isDirectValueOfRenderPropertyLoose(maybeParentComponent.value)) {
            context.report({
        ...
        for (const { name: name2, node: component } of classComponents) {
          if (eff.O.isNone(AST11__namespace.traverseUp(component, (node2) => isClassComponent11(node2) || isFunctionComponent(node2)))) {
            continue;
          }
          context.report({
            messageId: "nestedComponent",
...
var no_set_state_in_component_did_mount_default = createRule({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      ...
      if (eff.O.isNone(maybeParentMethod)) return eff.O.none();
      const parentMethod = maybeParentMethod.value;
      if (parentMethod.parent !== parentClass.body) return eff.O.none();
      if (context.sourceCode.getScope(node).upper !== context.sourceCode.getScope(parentMethod)) return eff.O.none();
      return eff.O.some({
  ...
  create(context) {
    ...
    return {
      ...
      JSXOpeningElement(node) {
        ...
        const maybeJSXValueAttribute = eff.O.fromNullable(
          node.attributes.find((attribute) => {
            return attribute.type === types.AST_NODE_TYPES.JSXAttribute && attribute.name.name === "value";
          })
        );
        if (eff.O.isNone(maybeJSXValueAttribute) || !("value" in maybeJSXValueAttribute.value)) return;
      ...
      VariableDeclarator(node) {
        ...
        if (node.init && AST11__namespace.isThisExpression(node.init) && node.id.type === types.AST_NODE_TYPES.ObjectPattern) {
          for (const prop of node.id.properties) {
            if (prop.type === types.AST_NODE_TYPES.Property && AST11__namespace.isKeyLiteralLike(prop, prop.key)) {
              eff.O.map(getName3(prop.key), (name2) => propertyUsages.get(currentClass)?.add(name2));
            }
          }
        }
      }
...
function check(node, context, allowExpressions) {
  const initialScope = context.sourceCode.getScope(node);
  if (JSX8__namespace.isKeyedElement(node, initialScope)) return;
  if (JSX8__namespace.isBuiltInElement(node.parent)) context.report({ messageId: "noUselessFragmentInBuiltIn", node });
  ...
  if (allowExpressions && node.children.length === 1 && JSX8__namespace.isLiteral(firstChildren) && !isChildren) return;
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        function isFunctionComponent(block) {
          ...
          return eff.O.isSome(maybeId) && core.isComponentName(maybeId.value.name) && components.some((component) => component.node === block);
        }
...
var prefer_shorthand_boolean_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      const { value } = node;
      const propName = JSX8__namespace.getPropName(node);
...
var prefer_shorthand_fragment_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      if (!core.isFragmentElement(node, context)) return eff.O.none();
  ...
  create(context) {
    ...
    return {
      JSXOpeningElement(node) {
        ...
        eff.O.map(getName5(node.name), (name2) => {
